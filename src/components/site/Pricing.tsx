...
const plans = [
  { name: "AcessoFast Team", users: "Até 10 usuários", sessions: "Até 5 acessos simultâneos por técnico", mensal: "R$ 249", anual: "R$ 2.988" },
  { name: "AcessoFast Business", users: "Até 20 usuários", sessions: "Até 10 acessos simultâneos por técnico", mensal: "R$ 449", anual: "R$ 5.388" },
  { name: "AcessoFast Scale", users: "Até 50 usuários", sessions: "Acessos simultâneos ilimitados por técnico", note: true, mensal: "R$ 899", anual: "R$ 10.788" },
  { name: "AcessoFast Enterprise", users: "Usuários sob medida", sessions: "Configuração personalizada", custom: true, mensal: "Sob consulta", anual: "Sob consulta" },
];
    ...
    <section id="preco" className="bg-surface py-28">
      <div className="mx-auto max-w-6xl px-6">
        <div className="text-center">
          ...
          <p className="mx-auto mt-4 max-w-xl text-lg text-text-muted">Preço fechado, sem excedente e sem reajuste surpresa. Endpoints ilimitados em todos os planos.</p>
          ...
          {plans.map((p) => (
            <div key={p.name} className="flex flex-col rounded-card border border-border bg-surface-2 p-6 shadow-soft">
              ...
              <div className="mt-4 flex items-baseline gap-1.5">
                <span className="text-3xl font-extrabold tracking-tight text-text">{billing === "anual" ? p.anual : p.mensal}</span>
                {!p.custom && <span className="text-sm text-text-muted">/{billing === "anual" ? "ano" : "mês"}</span>}
              </div>
              <ul className="mt-6 flex-1 space-y-3 text-left">
                ...
                {!p.custom && <li className="flex items-start gap-3"><span className="mt-0.5 flex h-5 w-5 flex-none items-center justify-center rounded-full bg-success/15 text-success"><Check className="h-3.5 w-3.5" strokeWidth={2.5} /></span><span className="text-[15px] text-text">Endpoints ilimitados</span></li>}
              </ul>
